...
function registerFrontDeskRoutes(app: Express) {
  ...
  app.post("/api/front-desk/walk-in",
    ...
    async (req: AuthRequest, res: Response) => {
      try {
        ...
        const { guest, roomId, roomTypeId, ratePlanId, nights, adults, children, specialRequests, depositAmount, paymentMethod } = req.body;

        // Validate required fields
        if (!guest?.firstName || !guest?.lastName) {
          return res.status(400).json({ error: "Guest first and last name required" });
        }
        if (!roomId) {
          return res.status(400).json({ error: "Room selection required" });
        }
        ...
        const folio = await storage.createFolio({
          ...
          totalPayments: depositAmount ? depositAmount.toString() : "0",
          balance: depositAmount ? (parseFloat(totalAmount) - parseFloat(depositAmount)).toFixed(2) : totalAmount
        });
  ...
  app.patch("/api/reservations/:id/stay-adjustment",
    ...
    async (req: AuthRequest, res: Response) => {
      try {
        const { id } = req.params;
        const { adjustmentType, newTime, additionalCharge, notes } = req.body;
        ...
        if (additionalCharge && parseFloat(additionalCharge) > 0) {
          const folio = await storage.getFolioByReservation(id);
          if (folio) {
            await storage.createCharge({
              folioId: folio.id,
